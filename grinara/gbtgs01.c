...
*  Modified for Game Shark, Joshua Neal                      *
*                                                            *
* ai[]=0 w a[7..0]                                           *
* ai[]=1 w a[15..8]                                          *
...
char got_gs=0;
unsigned char flashmfgr, flashtype;
...
//   adr<<=1;                     // adr x 2
   l=adr & 0xff;                // a7~a0
   m=(adr>>8) & 0x1f;           // a13~a8
   h=(adr>>13) & 0xff;          // a21~a13
//   if (h)
      m|=0x40;  // >bank 0

   set_adr(0x7FE1);             // write 2000:h
   set_data_write
   write_data(h);               // set rom bank value
//   outportb(data,h);          // set rom bank value
   set_ai_data(1,m);            // a[15..8]
   set_ai_data(0,l);            // a[7..0]
   out_byte(d);                  // write data to eeprom
}
...
char gs_erase(void)
{
	set_bank(0x7FE1,0x02);   // Datel MBC Bank
...
//j   out_byte_eeprom(data);       // write data to eeprom
...
char check_eeprom(void)
{
   ...
   set_adr(0x101);
   read_byte();
   /* should now be triggered so we can access cart directly */

   set_bank(0x7FE1,0x2);
   out_adr_data_32k(0x5555,0xaa);       // softwave product ID entry
   out_adr_data_32k(0x2aaa,0x55);
   out_adr_data_32k(0x5555,0x90);

   set_adr(0x0);                  /* adr2,adr1,adr0 */
   flashmfgr=read_byte();
   set_adr(0x1);                  /* adr2,adr1,adr0 */
   flashtype=read_byte();
   out_adr_data_32k(0xf0,0xf0);       // product ID
#if 0
   set_adr(0x1);                  /* adr2,adr1,adr0 */
   printf("Byte at 0x1 : %x\n",read_byte());
   set_adr(0x101);                  /* adr2,adr1,adr0 */
   printf("Byte at 0x101 : %x\n",read_byte());
#endif

   printf("Flash type: %x %x\n",flashmfgr, flashtype);
   if (flashmfgr == 0xbf && flashtype == 0xb5) {
	printf("Game Shark Found!\n");
	got_gs = 1;
	return(0);
   }
   return(1);
...
//j   for (j=0;j<64;j++){          // 16k bytes = 64 x 256 bytes
#if 0
   set_bank(0x7FE5,10);
   set_bank(0x7FE2,3);
   set_bank(0x7FE4,10);
#endif
   set_bank(0x7FE1,bank_16k << 1);   // for MCB1 16k bank
   for (j=0;j<32;j++){          // 16k bytes = 64 x 256 bytes
      if (bank_16k)
...
//       mix.buffer[idx+i]=inportb(data);
   ...
   set_bank(0x7FE1,(bank_16k << 1) + 1);   // for MCB1 16k bank
   for (j=0;j<32;j++){          // 16k bytes = 64 x 256 bytes
      if (bank_16k)
	 set_ai_data(1,(j|0x40));       // set adr[15..8]
      else
...
char gs_write_eeprom_16k(unsigned int bank_16k)
{
	printf("w");
	idx=0;

	for (j=0; j<8192 ;j++){          // 16k bytes


		set_bank(0x7FE1,0x2);   // Datel MBC Bank
		out_adr_data_32k(0x5555,0xaa);          /* 5555:aa softwave product ID entry */
		out_adr_data_32k(0x2aaa,0x55);          /* 2aaa:55 adr2,adr1,adr0,data */
		out_adr_data_32k(0x5555,0xA0);          /* 5555:90 adr2,adr1,adr0,data */
		set_bank(0x7FE1,bank_16k << 1);   // Datel MBC Bank
	...
	for (; j<16384 ;j++){          // 16k bytes

		set_bank(0x7FE1,0x2);   // Datel MBC Bank
		out_adr_data_32k(0x5555,0xaa);          /* 5555:aa softwave product ID entry */
		out_adr_data_32k(0x2aaa,0x55);          /* 2aaa:55 adr2,adr1,adr0,data */
		out_adr_data_32k(0x5555,0xA0);          /* 5555:90 adr2,adr1,adr0,data */
		set_bank(0x7FE1,(bank_16k << 1)+1);   // Datel MBC Bank
		out_adr_data_32k(0x4000+(j-8192),mix.buffer[j]);          /* 5555:aa softwave product ID entry */
		if (data_polling()){
			printf("write error!!!\n");
			return(-1);
		}
...
{
   if (got_gs)
      return(gs_write_eeprom_16k(bank_16k));
   return(-1);
...
//   fprintf(stderr, "Usage: %s [-option] <Filename>\n", progname);
   fprintf(stderr, "Usage: gbtgs [-option] <Filename>\n");
...
void main(int argc, char *argv[])
{
   ...
   init_port();
   printf("\n--- Parallel EPP/SPP Port to GB Xchanger program, Game Shark v0.01 ---\n");
   port[0]=peek(0x40,8);                // lpt1 base address
   port[1]=peek(0x40,10);               // lpt2 base address
   if (port[0]==0){
      printf("No Printer Port Avialable!\n");
      exit(1);
   }
   if (port[1]==0)
